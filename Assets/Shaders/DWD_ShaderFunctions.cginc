...
half Average(half3 base)
{
	return (base.x + base.y + base.z) * 0.333;
}
...
half Approximation(half ramp, half rim, half edge, half falloff, half power, half boost, half mask)
{
    return saturate((pow(saturate(rim - falloff), power) * boost) * mask * (1.0 - ramp) + edge);
}

half3 ColorizeApproximation(half3 color, half ramp, half rim, half edge, half falloff, half power, half boost, half mask)
{
	return color * saturate( (pow(saturate(rim - falloff), power) * boost) * mask * (1.0 - ramp) + edge).xxx;
}
...
#ifndef _HAS_DITHER
#define _HAS_DITHER 1

float FRDither8x8Bayer(int x, int y)
{
