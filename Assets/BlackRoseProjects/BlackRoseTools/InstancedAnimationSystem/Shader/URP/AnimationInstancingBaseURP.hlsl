...
half4 skinning(inout AttributesInstancing v)
{
	float4 w = v.color;
	...
	half4 localPosPreAniNext = mul(v.positionOS, localToWorldMatrixPreAniNext);
	...
	localPos = lerp(localPos, localPrePos, (1.0f - progress) * (preAniFrame > 0.0f));
	return localPos;
}

half4 skinningShadow(inout AttributesInstancing v)
{
	half4 bone = half4(v.texcoord2.x, v.texcoord2.y, v.texcoord2.z, v.texcoord2.w);
	float curFrame = UNITY_ACCESS_INSTANCED_PROP(AnimationProp, frameIndex);
	float preAniFrame = UNITY_ACCESS_INSTANCED_PROP(AnimationProp, preFrameIndex);
