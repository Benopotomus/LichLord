...
namespace LichLord.NonPlayerCharacters
{
    public class MeleeHitTrackerComponent : MonoBehaviour
    {
        ...
        // -----------------------------------------------------------------
        // NEW: Draw a wire sphere at `position` with `radius` for 2 seconds
        // -----------------------------------------------------------------
        public void DrawHitShape(Vector3 position, float radius)
        {
#if UNITY_EDITOR
            // Start a coroutine that draws every frame for 2 seconds
        ...
        private IEnumerator DrawShapeRoutine(Vector3 center, float radius, float duration)
        {
            ...
            while (Time.realtimeSinceStartup < endTime)
            {
                ...
                float alpha = Mathf.Clamp01(life / 0.3f); // fade last 0.3s
                Color color = new Color(1f, 0.5f, 0f, alpha * 0.7f); // orange, semi-transparent
        ...
        private void DrawWireSphere(Vector3 center, float radius, Color color)
        {
            ...
            DrawCircle(center, Vector3.up, radius, color, SEGMENTS);      // XY
            DrawCircle(center, Vector3.right, radius, color, SEGMENTS);   // YZ
            DrawCircle(center, Vector3.forward, radius, color, SEGMENTS); // XZ
        }
