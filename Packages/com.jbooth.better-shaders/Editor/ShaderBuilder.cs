...
namespace JBooth.BetterShaders
{
   ...
   public class ShaderBuilder
   {
      ...
      IPipelineAdapter GetAdapter(Options options, RenderPipeline rp)
      {
         ...
         else if (rp == RenderPipeline.URP2021)
         {
            return new PipelineURP2021();
         }
         else if (rp == RenderPipeline.URP2022)
         {
            return new PipelineURP2022();
         }
         else if (rp == RenderPipeline.URP2023)
         {
            return new PipelineURP2023();
         }
         ...
         else if (rp == RenderPipeline.HDRP2021)
         {
            return new PipelineHDRP2021();
         }
         else if (rp == RenderPipeline.HDRP2022)
         {
            return new PipelineHDRP2022();
         }
         else if (rp == RenderPipeline.HDRP2023)
         {
            return new PipelineHDRP2023();
         }
         else
         {
      ...
      static internal RenderPipeline GetRenderPipeline(RenderPipeline? forcedRP = null)
      {

#if USING_HDRP
#if UNITY_2023_3_OR_NEWER
          RenderPipeline rp = RenderPipeline.HDRP2023;
#elif UNITY_2022_2_OR_NEWER
            RenderPipeline rp = RenderPipeline.HDRP2022;
#elif UNITY_2021_2_OR_NEWER
         RenderPipeline rp = RenderPipeline.HDRP2021;
         ...
         RenderPipeline rp = RenderPipeline.HDRP2019;
            ...
            RenderPipeline rp = RenderPipeline.URP2023;
#elif UNITY_2022_2_OR_NEWER
            RenderPipeline rp = RenderPipeline.URP2022;
#elif UNITY_2021_2_OR_NEWER
            RenderPipeline rp = RenderPipeline.URP2021;
#elif UNITY_2020_2_OR_NEWER
      ...
      static internal RenderPipelineType GetRenderPipelineType(RenderPipeline? forcedRP)
      {
         RenderPipeline rp = GetRenderPipeline(forcedRP);
         if (rp == RenderPipeline.HDRP2019 || rp == RenderPipeline.HDRP2020 || rp == RenderPipeline.HDRP2021 || rp == RenderPipeline.HDRP2022 || rp == RenderPipeline.HDRP2023)
            return RenderPipelineType.HDRP;
         if (rp == RenderPipeline.URP2019 || rp == RenderPipeline.URP2020 || rp == RenderPipeline.URP2021 || rp == RenderPipeline.URP2022 || rp == RenderPipeline.URP2023)
         ...
         OptionOverrides optionOverrides = null)
      {
         ...
         else if (rp == RenderPipeline.URP2019 || rp == RenderPipeline.URP2020 || rp == RenderPipeline.URP2021 || rp == RenderPipeline.URP2022 || rp == RenderPipeline.URP2023)
         {
            defines.AppendLine( "\n   #define _URP 1");
         }
         else if (rp == RenderPipeline.Standard)
         {
            defines.AppendLine("\n   #define _STANDARD 1");
         }
         else
         {
            Debug.LogError("Render Pipeline Not Found : " + rp);
            defines.AppendLine("\n   #define _STANDARD 1");
         }
         ...
         string shaderTarget = "#pragma target 3.0";
         if (codeNoComments.Contains(".vertexID"))
         {
            shaderTarget = "#pragma target 3.5";
         }
         if (rp == RenderPipeline.HDRP2019 || rp == RenderPipeline.HDRP2020 || rp == RenderPipeline.HDRP2021 || rp == RenderPipeline.HDRP2022 || rp == RenderPipeline.HDRP2023)
         {
            shaderTarget = "#pragma target 4.5";
         }
         else if (rp == RenderPipeline.URP2023)
         {
            if (codeNoComments.Contains(".vertexID"))
            {
               shaderTarget = "#if (defined(SHADER_API_GLES) || defined(SHADER_API_GLES3)) \n            #pragma target 3.5\n#else\n            #pragma target 4.5\n#endif";
            }
            else
            {
               shaderTarget = "#if (defined(SHADER_API_GLES) || defined(SHADER_API_GLES3)) \n            #pragma target 3.0\n#else\n            #pragma target 4.5\n#endif";
         ...
         {
            shaderTarget = "#pragma target 4.6";
         }

         if (!string.IsNullOrEmpty(options.shaderTarget))
         {
            shaderTarget = "#pragma target " + options.shaderTarget;
         }
         ...
         else if (parser.blocks.customPassBlocks.ContainsKey("allbutmeta"))
         {
            StripPass(parser.blocks, ref shader, "%PASSFORWARD%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSFORWARDADD%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSGBUFFER%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSDEPTH%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSSHADOW%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSSELECT%", "allbutmeta");
            StripPass(parser.blocks, ref shader, "%PASSMETA%", "");
            StripPass(parser.blocks, ref shader, "%PASSMOTION%", "allbutmeta");
         }
         else
         {
