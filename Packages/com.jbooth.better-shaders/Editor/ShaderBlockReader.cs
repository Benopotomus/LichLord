...
namespace JBooth.BetterShaders
{
   ...
   internal class ShaderBlockReader
   {
      ...
      public void CombineBlockList(List<BlockData> data, ShaderBuilder.RenderPipeline? forcedRP)
      {
         List<string> onceBlocks = new List<string>();
         var rp = ShaderBuilder.GetRenderPipelineType(forcedRP);
         ...
         foreach (var b in data)
         {
            ...
            // filter out stackables for a single pipeline
            if (o.pipeline == "HDRP" && rp != ShaderBuilder.RenderPipelineType.HDRP)
               continue;
            else if (o.pipeline == "URP" && rp != ShaderBuilder.RenderPipelineType.URP)
               continue;
            else if (o.pipeline == "Standard" && rp != ShaderBuilder.RenderPipelineType.Standard)
               continue;
      ...
      public void Read(string assetPath, ShaderBuilder.RenderPipeline? forcedRP)
      {
         ReadIncludes(assetPath, 0);
         includes.Add(assetPath);
         List<BlockData> data = RenderIncludes();

         CombineBlockList(data, forcedRP);
      }


      public void ReadStack(ShaderBuilder.RenderPipeline? forcedRP = null)
      {
         includes.Reverse();
