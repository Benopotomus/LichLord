   ...
   public class BetterShaderImporterEditor : ScriptedImporterEditor
   {
      ...
      public override void OnInspectorGUI()
      {
         ...
         if (GUILayout.Button("Export All Shaders as TextAssets"))
         {
            ...
            EditorUtility.DisplayProgressBar("Exporting shaders","URP2019",0.15f);
            ExportShader(ShaderBuilder.RenderPipeline.URP2019,".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders","HDRP2019",0.30f);
            ExportShader(ShaderBuilder.RenderPipeline.HDRP2019,".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders", "URP2020 SRP", 0.45f);
            ExportShader(ShaderBuilder.RenderPipeline.URP2020, ".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders", "HDRP2020 SRP", 0.60f);
            ExportShader(ShaderBuilder.RenderPipeline.HDRP2020, ".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders", "URP2021 SRP", 0.75f);
            ExportShader(ShaderBuilder.RenderPipeline.URP2021, ".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders", "URP2022 SRP", 0.75f);
            ExportShader(ShaderBuilder.RenderPipeline.URP2022, ".txt", null, AssetDatabase.GetAssetPath(target));
            EditorUtility.DisplayProgressBar("Exporting shaders", "URP2023 SRP", 0.75f);
            ExportShader(ShaderBuilder.RenderPipeline.URP2023, ".txt", null, AssetDatabase.GetAssetPath(target));

            EditorUtility.DisplayProgressBar("Exporting shaders", "HDRP2021 SRP", 0.90f);
            ExportShader(ShaderBuilder.RenderPipeline.HDRP2021, ".txt", null, AssetDatabase.GetAssetPath(target));
      ...
      public static string BuildExportShader(ShaderBuilder.RenderPipeline selectedRenderPipeline, OptionOverrides overrides, string assetPath)
      {
         try
         {
            ...
            blocks.Read(assetPath, selectedRenderPipeline);
